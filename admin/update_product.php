...
if (isset($_POST['update'])) {
   ...
   $update_product = $connect->prepare("UPDATE `products` SET name = ?, category = ?, price = ? WHERE id = ?");
   ...
   if (!empty($image)) {
      if ($image_size > 2000000) {
         ...
         $update_image = $connect->prepare("UPDATE `products` SET image = ? WHERE id = ?");
...
<body>
   ...
   <section class="update-product">
      ...
      $show_products = $connect->prepare("SELECT * FROM `products` WHERE id = ?");

...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      $select_name = $connect->prepare("SELECT * FROM `admin` WHERE name = ?");
      $select_name->execute([$name]);
      if($select_name->rowCount() > 0){
         $message[] = 'username already taken!';
      }else{
         $update_name = $connect->prepare("UPDATE `admin` SET name = ? WHERE id = ?");
         $update_name->execute([$name, $admin_id]);
      }
   }

   $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709';
   $select_old_pass = $connect->prepare("SELECT password FROM `admin` WHERE id = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $connect->prepare("UPDATE `admin` SET password = ? WHERE id = ?");
